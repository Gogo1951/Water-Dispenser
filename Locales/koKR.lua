...
L["OPTIONS_DISPENSE_DESC"] =
	"거래 창이 열리면 자동으로 채웁니다. 아래 각 옵션은 개별적으로 켜거나 끌 수 있습니다."
...
L["OPTIONS_COMBAT_DESC"] =
	"인터페이스 오류를 방지하기 위해 전투 중에는 자동 채우기가 항상 일시 정지됩니다. 일시 정지 시 대화창에 알림이 표시됩니다. 거래 창이 열려있다면 전투가 끝난 직후 자동으로 재개됩니다."
...
L["OPTIONS_ITEMS_DESC"] =
	"각 아이템의 배분할 묶음 수를 설정합니다. 주의: 클래식 시대 서버 및 불타는 성전 서버는 자동 묶음 나누기를 지원하지 않습니다. 죄송합니다!"
...
L["OPTIONS_ITEM_KEEP_AT_LEAST_DESC"] =
	"가방에 항상 설정한 수량만큼 유지합니다. 이 수를 초과한 개수만 상대방에게 줄 수 있는 것으로 간주합니다."
...
L["OPTIONS_ANNOUNCEMENTS_DESC"] =
	"Water Dispenser는 나눠줄 수 있는 항목을 알리는 매크로를 생성할 수 있습니다. 상황에 맞는 채널(/일반, /파티, /공격대)을 자동으로 선택하고 가방의 최신 수량을 사용합니다."
