...
local function Refresh()
    if diagnosticsAppName then
        AceConfigRegistry:NotifyChange(diagnosticsAppName)
    end
end
...
local function BuildDiagnosticsOptions()
    return {
        type = "group",
        name = D.TAB,
        args = {
            descWarning = Desc(D.WARNING, 1),
            spaceEnable = Spacer(2),
            toggleEnable = {
                type = "toggle",
                name = D.ENABLE,
                width = "full",
...
function ns.InitOptionsDiagnostics(appName, parentDisplayName)
    diagnosticsAppName = appName
    AceConfig:RegisterOptionsTable(appName, BuildDiagnosticsOptions())
    AceConfigDialog:AddToBlizOptions(appName, D.TAB, parentDisplayName)
end
