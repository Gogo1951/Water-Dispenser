...
-- Environment probing and state capture for bug reports. Read-only and built
-- only on a button press, except the Taint Log button which sets a CVar.
...
-- Developer-facing strings, deliberately not localized. Kept here, never in
-- Locales/.
...
-- Per-argument byte cap. 255 holds a full item link while still bounding a
-- runaway argument.
local EVENT_LOG_MAX_ARG_LENGTH = 255

-- Noisy events the logger skips. BAG_UPDATE fires per slot change;
-- BAG_UPDATE_DELAYED (once per settle) is kept.
...
ns.DIAGNOSTIC_API_CHECKS = {
    -- { label, testFunction }
    {"Enum.ItemClass.Consumable", function() return type(Enum) == "table" and type(Enum.ItemClass) == "table" and Enum.ItemClass.Consumable ~= nil end},
    {"LE_ITEM_CLASS_CONSUMABLE (legacy)", function() return LE_ITEM_CLASS_CONSUMABLE ~= nil end},
    ...
    {"TradeFrame_GetAvailableSlot", function() return type(TradeFrame_GetAvailableSlot) == "function" end},
    {"ClickTradeButton", function() return type(ClickTradeButton) == "function" end},
    {"MAX_TRADABLE_ITEMS", function() return MAX_TRADABLE_ITEMS ~= nil end},
    ...
    {"C_Timer.NewTimer", function() return type(C_Timer) == "table" and type(C_Timer.NewTimer) == "function" end},
...
-- Probes the state behind a "nothing fills" report: player class, trade
-- partner class/level, resolved scope, and per-item active/count. Read-only.
...
function ns:BuildContextReport()
    ...
    lines[#lines + 1] = string.format(
        "Player: %s (%s), level %s",
        tostring(className), tostring(classToken), tostring(UnitLevel("player"))
    )
    ...
    if trade and trade.Active then
        lines[#lines + 1] = string.format(
            ...
            tostring(trade.Class), tostring(trade.Level), tostring(trade.Party)
        )
        local npcClassLocalized, npcClassToken = UnitClass("NPC")
        lines[#lines + 1] = string.format(
            "Trade (live): UnitClass(\"NPC\")=%s/%s UnitLevel(\"NPC\")=%s",
            tostring(npcClassLocalized), tostring(npcClassToken), tostring(UnitLevel("NPC"))
        )
    else
    ...
    if activePartner then
        lines[#lines + 1] = string.format("Resolved scope: %s (active trade partner class: %s)", scope, tostring(activePartner))
    else
    ...
    if db then
        lines[#lines + 1] = string.format(
            ...
            tostring(db.Dispense), tostring(db.DispenseSolo), tostring(db.DispenseGroup), tostring(db.DispenseRaid)
        )
        lines[#lines + 1] = string.format(
            "Would auto-fill for resolved scope %s: %s",
            scope, tostring((db.Dispense and db["Dispense" .. scope]) and true or false)
        )
    else
    ...
    if ns.DB and ns.DB.Items then
        local keys = {}
        for key in pairs(ns.DB.Items) do
            keys[#keys + 1] = key
        end
        ...
        for _, key in ipairs(keys) do
            ...
            lines[#lines + 1] = string.format(
                ...
                tostring(itemConfig.Name or key), tostring(active), scope, table.concat(parts, " ")
            )
        end
    else
    ...
    for _, spell in ipairs(ns.DIAGNOSTIC_SPELLS) do
        ...
        lines[#lines + 1] = string.format(
            "  [%s] %s (%d) (IsSpellKnown=%s, IsPlayerSpell=%s)",
            verdict, label, id, rawKnown, rawPlayer
        )
    end
...
local function DumpTable(value, indent, depth, lines)
    ...
    -- Summarize oversized tables by length so a large custom item list
    -- can't blow up the dump.
    if #keys > 20 then
...
function ns:BuildSavedVariablesReport()
    ...
    lines[#lines + 1] = "WaterDispenserCharDB = {"
    DumpTable(WaterDispenserCharDB or {}, "    ", 1, lines)
    lines[#lines + 1] = "}"
    return table.concat(lines, "\n")
end
...
-- The taintLog CVar logs UI taint to Logs\taint.log: 2 = verbose, 0 = off.
-- The only state the diagnostics panel writes.
